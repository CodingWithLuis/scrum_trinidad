...
export default function CategoryCreate() {
    ...
    const checkCategoryExists = async (name) => {
        const response = await fetch(`/categories/checkDuplicate?name=${name}`);
    ...
    const validationSchema = Yup.object({
        name: Yup.string().min(2, 'El nombre debe tener al menos 2 caracteres')
            .max(50, 'El nombre de la categoria no puede superar los 50 caracteres')
            .required('La categoria es un campo obligatorio')
            .test("unique", "Este categoria ya está registrada", async (value) => {
                if (!value) return false;
                return await checkCategoryExists(value);
            }),
        description: Yup.string().min(5, 'La descripción debe tener al menos 5 caracteres')
            .max(100, 'El campo descripcion no puede tener mas de 100 caracteres')
